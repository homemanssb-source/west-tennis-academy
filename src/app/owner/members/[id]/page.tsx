...
export default function MemberDetailPage() {
  ...
  if (loading) return <div style={{ padding: '4rem', textAlign: 'center', color: '#9ca3af' }}>불러오는 중...</div>
  if (!member) return <div style={{ padding: '4rem', textAlign: 'center', color: '#9ca3af' }}>회원을 찾을 수 없습니다</div>
  ...
  return (
    <div style={{ background: '#f9fafb', minHeight: '100vh' }}>
      <div style={{ background: 'white', borderBottom: '1.5px solid #f3f4f6', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', position: 'sticky', top: 0, zIndex: 40 }}>
        <Link href="/owner/members" style={{ color: '#9ca3af', textDecoration: 'none', fontSize: '1.25rem' }}>‹</Link>
        <h1 style={{ fontFamily: 'Oswald, sans-serif', fontSize: '1.25rem', fontWeight: 700, color: '#111827' }}>회원 상세</h1>
        <span style={{ marginLeft: 'auto', background: member.is_active ? '#dcfce7' : '#fee2e2', color: member.is_active ? '#15803d' : '#b91c1c', fontSize: '0.75rem', fontWeight: 700, padding: '3px 10px', borderRadius: '9999px' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <div style={{ background: 'white', borderRadius: '1rem', border: '1.5px solid #f3f4f6', padding: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
            ...
            <div>
              <label style={{ fontSize: '0.75rem', fontWeight: 700, color: '#6b7280', display: 'block', marginBottom: '6px' }}>등록일</label>
              <div style={{ fontSize: '0.875rem', color: '#374151', padding: '0.75rem 1rem', background: '#f9fafb', borderRadius: '0.75rem' }}>{new Date(member.created_at).toLocaleDateString('ko-KR')}</div>
            </div>
          </div>
          <button onClick={handleSave} disabled={saving}
            style={{ width: '100%', marginTop: '1rem', padding: '0.75rem', borderRadius: '0.75rem', border: 'none', background: '#16A34A', color: 'white', fontWeight: 700, cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif' }}>
            {saving ? '저장 중...' : '저장'}
          </button>
        </div>
        ...
        {/* PIN·계정 관리 */}
        <div style={{ background: 'white', borderRadius: '1rem', border: '1.5px solid #f3f4f6', padding: '1.25rem' }}>
          <h2 style={{ fontFamily: 'Oswald, sans-serif', fontSize: '1rem', fontWeight: 700, marginBottom: '1rem', color: '#111827' }}>계정 관리</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <button onClick={handleResetPin}
              style={{ padding: '0.75rem', borderRadius: '0.75rem', border: '1.5px solid #e5e7eb', background: 'white', color: '#374151', fontWeight: 600, cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif', textAlign: 'left' }}>
              🔑 PIN 초기화 (123456으로 재발급)
            </button>
            <button onClick={handleToggle}
              style={{ padding: '0.75rem', borderRadius: '0.75rem', border: `1.5px solid ${member.is_active ? '#fecaca' : '#86efac'}`, background: member.is_active ? '#fef2f2' : '#f0fdf4', color: member.is_active ? '#b91c1c' : '#15803d', fontWeight: 600, cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif', textAlign: 'left' }}>
              {member.is_active ? '🚫 계정 비활성화' : '✅ 계정 활성화'}
            </button>
          </div>
        </div>
      </div>
