...
export default function LessonPlanPage() {
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch('/api/lesson-plans', {
      ...
      body: JSON.stringify({ member_id: memberId, coach_id: coachId, month_id: monthId, lesson_type: lessonType, unit_minutes: unitMin, schedules, amount: Number(amount) || 0 }),
    })
  ...
  return (
    <div style={{ background: '#f9fafb', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <div style={{ background: 'white', borderRadius: '1rem', border: '1.5px solid #f3f4f6', padding: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
            <div>
              ...
              <select className="input-base" value={memberId} onChange={e => setMemberId(e.target.value)}>
            ...
            <div>
              <label style={{ fontSize: '0.75rem', fontWeight: 700, color: '#6b7280', display: 'block', marginBottom: '6px' }}>레슨 종류</label>
              <input className="input-base" value={lessonType} onChange={e => setLessonType(e.target.value)} placeholder="개인레슨, 그룹레슨 등" />
            </div>
            <div>
        ...
        {/* 수업 단위 */}
        <div style={{ background: 'white', borderRadius: '1rem', border: '1.5px solid #f3f4f6', padding: '1.25rem' }}>
          <h2 style={{ fontFamily: 'Oswald, sans-serif', fontSize: '1rem', fontWeight: 700, marginBottom: '1rem', color: '#111827' }}>수업 단위</h2>
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            ...
            <input type="number" placeholder="직접입력" value={UNITS.includes(unitMin) ? '' : unitMin}
        ...
        <p style={{ textAlign: 'center', fontSize: '0.75rem', color: '#9ca3af' }}>등록 후 스케줄 페이지로 이동합니다</p>
      </div>
    </div>
  )
}
