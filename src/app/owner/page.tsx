...
export default async function OwnerDashboard() {
  ...
  const menus = [
    ...
    { emoji: '💰', label: '납부 관리',     sub: '납부 현황·상태 변경',   href: '/owner/payment',      badge: unpaidCount ?? 0, badgeColor: '#b91c1c', badgeBg: '#fef2f2' },
    { emoji: '⚙️', label: '시스템 설정',  sub: 'PIN 발급·월 관리',      href: '/owner/settings',     badge: 0, badgeColor: '', badgeBg: '' },
  ]
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '0.75rem', marginBottom: '1.5rem' }}>
          {stats.map(s => (
            <Link key={s.label} href={s.href} style={{ textDecoration: 'none' }}>
              <div style={{ background: s.bg, border: `1.5px solid ${s.color}22`, borderRadius: '1rem', padding: '1rem', textAlign: 'center', transition: 'transform .15s' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '0.75rem' }}>
          {menus.map(m => (
            <Link key={m.label} href={m.href} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'white', border: '1.5px solid #f3f4f6', borderRadius: '1rem', padding: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.875rem', transition: 'box-shadow .15s', boxShadow: '0 1px 3px rgba(0,0,0,.05)' }}>
