        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button onClick={() => setTab('pending')}
            style={{ flex: 1, padding: '0.5rem', borderRadius: '0.75rem', border: 'none', cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif', fontWeight: 700, fontSize: '0.85rem',
              background: tab === 'pending' ? '#16A34A' : '#f3f4f6', color: tab === 'pending' ? 'white' : '#6b7280' }}>
            확인 대기 {pending.length > 0 && `(${pending.length})`}
          </button>
          <button onClick={() => setTab('done')}
            style={{ flex: 1, padding: '0.5rem', borderRadius: '0.75rem', border: 'none', cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif', fontWeight: 700, fontSize: '0.85rem',
              background: tab === 'done' ? '#6b7280' : '#f3f4f6', color: tab === 'done' ? 'white' : '#6b7280' }}>
            처리 완료
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: '1.25rem', paddingBottom: '6rem' }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
            {(tab === 'pending' ? pending : done).map(a => (
              <div key={a.id}
                onClick={() => tab === 'pending' ? (setSelected(a), setNote('')) : null}
                style={{ background: 'white', border: `1.5px solid ${tab === 'pending' ? '#fde68a' : '#f3f4f6'}`, borderRadius: '1rem', padding: '1rem 1.25rem', cursor: tab === 'pending' ? 'pointer' : 'default' }}>
                ...
                {tab === 'pending' && (
                  <div style={{ marginTop: '8px', fontSize: '0.75rem', color: '#854d0e', fontWeight: 600 }}>탭하여 수락/거절 →</div>
                )}
                {a.coach_note && (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
          ...
          <div style={{ background: 'white', width: '100%', maxWidth: '390px', borderRadius: '1.5rem 1.5rem 0 0', padding: '1.5rem' }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <input
                style={{ width: '100%', padding: '0.625rem 0.75rem', border: '1.5px solid #e5e7eb', borderRadius: '0.625rem', fontSize: '0.875rem', fontFamily: 'Noto Sans KR, sans-serif', boxSizing: 'border-box' as const }}
