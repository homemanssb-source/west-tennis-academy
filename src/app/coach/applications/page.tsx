...
export default function CoachApplicationsPage() {
  ...
  return (
    <div className="mobile-wrap" style={{ background: '#f9fafb', minHeight: '100dvh' }}>
      {/* 헤더 */}
      <div style={{ background: 'white', borderBottom: '1.5px solid #f3f4f6', padding: '1rem 1.25rem', position: 'sticky', top: 0, zIndex: 40 }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          ...
          <button onClick={() => setTab('done')} style={s.tab(tab === 'done', '#6b7280')}>처리 완료</button>
        </div>
      </div>

      {/* 콘텐츠 — CoachBottomNav 스페이서가 하단 여백 처리 */}
      <div style={{ padding: '1.25rem' }}>
        {tab === 'pending' && pending.length > 0 && (
          <div style={{ background: 'white', border: '1.5px solid #e5e7eb', borderRadius: '0.875rem', padding: '0.75rem 1rem', marginBottom: '0.875rem', display: 'flex', alignItems: 'center', gap: '0.625rem', minHeight: '3rem' }}>
            ...
            {checkedIds.size > 0 && (
              <>
                <button onClick={() => { setBulkModal('approve'); setBulkNote('') }}
                  style={{ padding: '0.5rem 0.875rem', background: '#16A34A', color: 'white', border: 'none', borderRadius: '0.5rem', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif', minHeight: '2.5rem' }}>
                  일괄 승인
                </button>
                <button onClick={() => { setBulkModal('reject'); setBulkNote('') }}
                  style={{ padding: '0.5rem 0.875rem', background: '#fef2f2', color: '#b91c1c', border: '1.5px solid #fecaca', borderRadius: '0.5rem', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif', minHeight: '2.5rem' }}>
                  일괄 거절
                </button>
              </>
            )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
            {current.map(a => (
              <div key={a.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.625rem' }}>
                ...
                <div style={{ flex: 1, background: 'white', border: `1.5px solid ${tab === 'pending' ? '#fde68a' : '#f3f4f6'}`, borderRadius: '1rem', padding: '1rem 1.25rem', cursor: tab === 'pending' ? 'pointer' : 'default', minHeight: '3.5rem' }}
                  onClick={() => tab === 'pending' ? (setSelected(a), setNote('')) : undefined}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '4px', flexWrap: 'wrap' }}>
                    ...
                    {a.status !== 'pending_coach' && (
                      <span style={{ marginLeft: 'auto', fontSize: '0.7rem', fontWeight: 700, padding: '2px 8px', borderRadius: '9999px', background: a.status === 'rejected' ? '#fee2e2' : '#dcfce7', color: a.status === 'rejected' ? '#b91c1c' : '#15803d' }}>
                  ...
                  {tab === 'pending' && <div style={{ marginTop: '8px', fontSize: '0.75rem', color: '#854d0e', fontWeight: 600 }}>탭하여 승인/거절 →</div>}
                  {a.coach_note && (
      ...
      {/* ✅ CoachBottomNav 컴포넌트 사용 (safe-area + 레이블 수정 자동 반영) */}
      <CoachBottomNav />
      ...
      {selected && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
          ...
          <div style={{ background: 'white', width: '100%', maxWidth: '480px', borderRadius: '1.5rem 1.5rem 0 0', padding: '1.25rem 1.5rem', paddingBottom: 'calc(1.5rem + env(safe-area-inset-bottom, 0px))' }}>
            ...
            <div style={{ display: 'flex', gap: '0.625rem' }}>
              <button onClick={() => handleAction('coach_approve')} disabled={saving}
                style={{ flex: 2, padding: '0.875rem', background: '#16A34A', color: 'white', border: 'none', borderRadius: '0.875rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif', minHeight: '3rem' }}>
                {saving ? '처리 중..' : '✅ 승인 → 관리자 전달'}
              </button>
              <button onClick={() => handleAction('coach_reject')} disabled={saving}
                style={{ flex: 1, padding: '0.875rem', background: '#fef2f2', color: '#b91c1c', border: '1.5px solid #fecaca', borderRadius: '0.875rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif', minHeight: '3rem' }}>
                거절
              </button>
            </div>
          </div>
          ...
          <div style={{ background: 'white', width: '100%', maxWidth: '480px', borderRadius: '1.5rem 1.5rem 0 0', padding: '1.25rem 1.5rem', paddingBottom: 'calc(1.5rem + env(safe-area-inset-bottom, 0px))' }}>
            ...
            <div style={{ display: 'flex', gap: '0.625rem' }}>
              <button onClick={() => setBulkModal(null)}
                style={{ flex: 1, padding: '0.875rem', background: 'white', color: '#6b7280', border: '1.5px solid #e5e7eb', borderRadius: '0.875rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif', minHeight: '3rem' }}>
                취소
              </button>
              <button onClick={() => handleBulkAction(bulkModal === 'approve' ? 'coach_approve' : 'coach_reject')} disabled={saving}
                style={{ flex: 2, padding: '0.875rem', background: bulkModal === 'approve' ? '#16A34A' : '#b91c1c', color: 'white', border: 'none', borderRadius: '0.875rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif', minHeight: '3rem' }}>
